...
export const TESSIE_FIELD_WEIGHTS = {
  // Lightweight fields
...
export class TessieQueryOptimizer {

  analyzeQuery(operation: string, parameters: any, estimatedResults = 1): QueryMetrics {
    let complexity = 1;
  ...
  private calculateDateRange(startDate?: string, endDate?: string): number {
    if (!startDate || !endDate) return 7; // Default 1 week
  ...
  private extractTimeFrame(query: string): { start_date?: string; end_date?: string } {
    const now = new Date();
    const lowerQuery = query.toLowerCase();
