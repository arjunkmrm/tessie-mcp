...
class DriveAnalyzer {
    ...
    mergeDrives(drives) {
        ...
        const mergedDrives = [];
        let currentGroup = [sortedDrives[0]];
    ...
    shouldMergeDrives(prevDrive, currentDrive, gapMinutes) {
    ...
    analyzeBatteryConsumption(drive) {
    ...
    generateDriveSummary(drive, battery, fsd) {
        ...
        if (stopTime > 1) {
            ...
            if (drive.stops.length > 0) {
                ...
                if (chargingStops > 0)
                    summary += ` (${chargingStops} charging stop${chargingStops > 1 ? 's' : ''})`;
                if (shortStops > 0)
                    summary += ` (${shortStops} short stop${shortStops > 1 ? 's' : ''})`;
            }
            summary += `\n`;
        }
...
exports.DriveAnalyzer = DriveAnalyzer;
//# sourceMappingURL=drive-analyzer.js.map